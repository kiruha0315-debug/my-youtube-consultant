...
st.title("🚀 YouTube AI 運営司令塔 (競合自動リサーチ版)")
...
if st.button("🛰️ 自動リサーチ＆戦略立案を開始"):
    if not yt_key or not gr_key or not hf_key or not my_url:
        ...
        try:
            ...
            # --- STEP 2: 競合を自動検索 ---
            with st.spinner("🕵️ 市場から競合ライバルを自動選別中..."):
                search_query = my_df['タイトル'].iloc[0][:15] # 最新動画のタイトルを元に検索
                search_res = youtube.search().list(q=search_query, type="video", part="snippet", maxResults=3, order="viewCount").execute()
            ...
            # --- STEP 3: コメント分析 & 戦略立案 ---
            with st.spinner("🧠 AIが勝機を見い出しています..."):
                ...
                1. 競合（{', '.join(rival_names)}）と比較して、自社の動画が勝っている点と不足している点。
                2. 競合動画のタイトル傾向から見た「次に狙い目のキーワード」。
                3. クリック率を上げるサムネイル用パワーワード（5案）。
                4. 視聴維持率を上げるための冒頭のフック（3パターン）。
                5. 次回動画の構成（導入・本編・エンディング）。
                """
            ...
            # --- STEP 4: 画像生成 ---
            with st.spinner("🎨 AIがサムネイル案を作成中..."):
                image_bytes = generate_image(f"Eye-catching YouTube thumbnail about {my_df['タイトル'].iloc[0]}", hf_key)
            ...
            with t1:
                st.markdown(report)
            with t2:
                fig = px.bar(all_df, x='再生数', y='タイトル', color='区分', barmode='group', orientation='h', title="競合との再生数比較")
